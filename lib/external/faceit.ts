...
/** Lifetime CS2 totals, or null if the player has no CS2 stats. */
export async function getLifetimeStats(
  playerId: string,
  opts: FaceitClientOptions = {},
): Promise<FaceitLifetime | null> {
  const data = await getJson(
    `${API}/players/${encodeURIComponent(playerId)}/stats/cs2`,
    lifetimeSchema,
    {
      service: "faceit",
      headers: auth(opts),
  ...
  return {
    matches: l.Matches,
    winRatePct: l["Win Rate %"],
    adr: l.ADR ?? null,
    kd: l["Average K/D Ratio"],
  };
}
