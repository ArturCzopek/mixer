...
| [ ] **M1-6** | Variant generation: server action fetches FACEIT data, computes S, stores `skill_snapshot`, creates 3 variants; admin preview, re-roll, publish; per-variant UI (lineups sorted by S, S breakdown, win %; no duo badges, D28) + **"How was this calculated?" panel** visible to everyone (E / F / M / A per player with the M1-4b details, cost breakdown, duo status, config used) | M1-4b, M1-5 | Generating for 10 real players shows 3 distinct variants; snapshot stored; re-roll never repeats a shown split |
...
| [ ] **M3-2** | Leetify client + FACEIT / Premier tabs (live, never stored, metrics shown as-is, "Data Provided by Leetify" logo) + **Leetify preview card** as a curiosity: tap a player in the lobby / variant or open a profile → server route fetches `/v3/profile` live (5–10 min HTTP cache) and shows Leetify's own ranks, ratings (aim, positioning, utility, clutch, opening) and win rate with their names and scales, plus the logo; private profiles say so. Never stored, never used in balancing | M3-1, P0-2 | Filter Mix / FACEIT / Premier works; preview card renders for a real player and for a private one; no Leetify data in DB (checked in review) |
...
**Unblocked right now (no owner input needed):** T-2, M1-1 (Steam login; preview deploy needs the Vercel env vars), M1-G, P0-4.
